...
for j in range(len(data_files)):
    ...
    file_bond_name = open(poly_name+"_tot_bond_type_rem_dup_again.txt", "r").readlines()
    file_out = open(poly_name+"_tot_bond_type_rem_rep.txt", "w")
...
### saving the atoms info in arrays######
    ...
    for i in range(len(name)):
     for j in range(len(name)):
       if j > i:
         if name_part1[i] == name_part2[j] and name_part1[j] == name_part2[i]:
           count_1 = num[i]
           count_2 = num[j]
